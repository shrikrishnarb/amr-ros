# amr_fleet_management.launch.py
#
# AMR Fleet Management launch — two modes:
#
#   Fleet-only mode (default, launch_gazebo:=false):
#     Assumes display.launch.py + spawn_agv.launch.py are already running.
#     Starts: Nav2 stack + nav2_goal_bridge + fleet_manager only.
#     Usage (3-terminal workflow):
#       Terminal 1: ros2 launch amr_description display.launch.py namespace:=agv1
#       Terminal 2: ros2 launch amr_description spawn_agv.launch.py namespace:=agv2 x:=2.0 y:=2.0
#       Terminal 3: ros2 launch amr_description amr_fleet_management.launch.py num_agvs:=2 spawn_poses:="0.0,0.0;2.0,2.0"
#
#   Standalone mode (launch_gazebo:=true):
#     Self-contained: starts Gazebo, spawns robots, all per-robot nodes, and fleet_manager.
#     Usage:
#       ros2 launch amr_description amr_fleet_management.launch.py num_agvs:=2 launch_gazebo:=true spawn_poses:="0.0,0.0;2.0,2.0"
#
# Arguments:
...
def launch_setup(context, *args, **kwargs):
    ...
    if not os.path.exists(tasks_file):
        raise RuntimeError(f'[amr_fleet_management] tasks.yaml not found: {tasks_file}')
    if not os.path.exists(nav2_params_template):
        raise RuntimeError(f'[amr_fleet_management] nav2_params_amr.yaml not found: {nav2_params_template}')
    if not os.path.exists(map_file):
        raise RuntimeError(f'[amr_fleet_management] my_map.yaml not found: {map_file}')
    ...
    nodes.append(LogInfo(msg=f'[amr_fleet_management] Mode: {mode}, robots: {robot_namespaces}'))
    ...
    # read TF from /{ns}/tf.  But display.launch.py / spawn_agv.launch.py start
    # odom_sim_filter and robot_state_publisher without TF remapping, so they
    # publish to the global /tf topic.  The relay bridges this mismatch without
    # requiring any changes to the existing launch files.
    # -----------------------------------------------------------------------
    ...
    #
    # Nav2 replaces ground_truth_waypoint_follower: it handles path planning,
    # dynamic obstacle avoidance, and publishes /<ns>/cmd_vel directly.
    # nav2_goal_bridge bridges /<ns>/goal_pose (from fleet_manager) to the
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description=(
                ...
                'Default false: assumes display.launch.py and spawn_agv.launch.py are already running.'
            ),
        ),
        DeclareLaunchArgument(
            'spawn_poses',
