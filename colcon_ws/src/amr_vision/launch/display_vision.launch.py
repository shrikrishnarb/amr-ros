...
def generate_launch_description():
    ...
    pkg_amr = FindPackageShare('amr_vision')  # CHANGE 1

    xacro_file = PathJoinSubstitution([
        pkg_amr, 'urdf', 'amr_vision.urdf.xacro'  # CHANGE 2
    ])
    ...
    # The namespace arg is forwarded to xacro so the camera Gazebo plugin
    # receives the correct namespace for its topic remappings.  # CHANGE 3
    robot_description = Command([
        'xacro ', xacro_file, ' namespace:=', namespace
    ])

    world = os.path.join(
        get_package_share_directory('amr_vision'),  # CHANGE 4
        'worlds', 'map.world'
    )
